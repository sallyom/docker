...
func Pull(ref reference.Named, imagePullConfig *ImagePullConfig) error {
	...
	if registry.IsReferenceFullyQualified(ref) {
		return pullFromRegistry(ref, imagePullConfig)
	}
	...
	for _, r := range registry.RegistryList {
		...
		if err = pullFromRegistry(fqr, imagePullConfig); err == nil {
			return nil
		}
	}
	return err
...
func pullFromRegistry(ref reference.Named, imagePullConfig *ImagePullConfig) error {
	var (
	...
	out.Write(sf.FormatStream(fmt.Sprintf("Trying to pull repository %s ...\n", repoInfo.CanonicalName)))
	...
	logName := registry.NormalizeLocalReference(ref, false)
